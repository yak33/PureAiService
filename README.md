...
- **自定义模型顺序**: 在模型管理页面可以通过拖拽调整已选模型的显示顺序
- **实时排序预览**: 拖拽后立即看到新的排序，并显示序号
- **快捷移除功能**: 可以直接在排序列表中移除不需要的模型
- **持久化保存**: 保存配置后，所有页面的模型下拉列表将按照你设置的顺序显示
...
5. 在右侧"已选模型"区域，可以**拖拽模型**调整显示顺序（拖动左侧图标即可）
6. 点击"保存配置"按钮
